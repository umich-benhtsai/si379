...
function drawBoard() {
  ...
  const NUM_BALLS = parseInt(ballCountInput.value);
  ...
  async function dropBall() {
    ...
    const randomColor = chroma(BALL_COLOR)
    ...
    for (let i = 0; i < NUM_LEVELS - 1; i++) {
      ...
      //   }
      const colorScale = chroma.scale("Spectral").domain([0, 1]);
      const hitCount = hitCounts[row][col];
      const color = colorScale(hitCount / NUM_BALLS).hex();
